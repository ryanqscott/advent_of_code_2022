...
for (let assignment of cleanupPair) {
    ...
    // checking for any intersections of the range
    if (parseInt(firstAssignment[1]) >= parseInt(secondAssignment[0]) && parseInt(firstAssignment[1]) <= parseInt(secondAssignment[1])) {
        numOfOverlappingAssignments++;
    } else if (parseInt(secondAssignment[1]) >= parseInt(firstAssignment[0]) && parseInt(secondAssignment[1]) <= parseInt(firstAssignment[1])) {
        numOfOverlappingAssignments++;
    }
}
